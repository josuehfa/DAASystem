...
PYBIND11_MODULE(daidalus_wrapper, m)
{
    ...
    py::class_<Position>(m, "Position")
        ...
        .def_static("makeLatLonAlt",
            (larcfm::Position (Position::*)
            (double, std::string, double, std::string, double, std::string))
    ...
    py::class_<Daidalus>(m, "Daidalus")
        ...
        //.def_property_readonly("current_time_", &Daidalus::current_time_)
        ...
        .def("setOwnshipState", 
            (void (Daidalus::*)
            (const std::string &, const Position&, const Velocity&, double))
            &Daidalus::setOwnshipState, "Clear all aircraft and set ownship state and current time.")
        //.def("addTrafficState",&Daidalus::addTrafficState)
